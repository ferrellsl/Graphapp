 ...
 *  Platform: macOS.
 *
 *  A Graphics object is a drawing state (colour, font, clip region, ...)
 *  attached to a target: a window's surface, a bitmap's surface, or an
 *  image.  The drawing operations themselves are in drawops.c.
 */
...
static Graphics *app_new_graphics(App *app, int kind, Surface *surf)
{
	Graphics *g;
	...
	graphics_extra(g)->surf = surf;

	return g;
}

static void app_set_surface_ops(Graphics *g, int kind)
{
	if (kind == GK_BITMAP) {
...
{
	return app_get_window_graphics(w);
}
...
Graphics *app_get_window_graphics(Window *w)
{
	...
	g = app_new_graphics(w->app, GK_WINDOW, &win_extra(w)->surf);
	g->win = w;
	g->area = app_get_window_area(w);
	app_set_clip_region(g, NULL);
	app_set_surface_ops(g, GK_WINDOW);
	app_set_rgb(g, BLACK);
	return g;
}
...
Graphics *app_get_bitmap_graphics(Bitmap *b)
{
	...
	g = app_new_graphics(b->win->app, GK_BITMAP, &bitmap_extra(b)->surf);
	g->bmap = b;
	g->area = app_get_bitmap_area(b);
	app_set_clip_region(g, NULL);
	app_set_surface_ops(g, GK_BITMAP);
	app_set_rgb(g, BLACK);
	return g;
}
...
Graphics *app_get_image_graphics(Image *img)
{
	...
	g = app_new_graphics(NULL, GK_NONE, NULL);
	g->img = img;
...
{
	/* Push anything drawn on a window to the screen. */
	if (graphics_extra(g)->kind == GK_WINDOW)
		app_flush_window(g->win);
...
void app_set_rgb(Graphics *g, Colour c)
{
	...
	if (g->img) {
		...
		eff = c;
		if (g->xor_mode) {
			eff.red   ^= graphics_extra(g)->bg.red;
			eff.green ^= graphics_extra(g)->bg.green;
			eff.blue  ^= graphics_extra(g)->bg.blue;
		}
		graphics_extra(g)->pixval =
			((uint32_t) eff.red << 16) |
			((uint32_t) eff.green << 8) | eff.blue;
		g->pixval = (long) graphics_extra(g)->pixval;
	}
...
/*
 *  Set the drawing colour by palette index.  Only images are indexed;
 *  windows and bitmaps have no palette on this platform.
 */
